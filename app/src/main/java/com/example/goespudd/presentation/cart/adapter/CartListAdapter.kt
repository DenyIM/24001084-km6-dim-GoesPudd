package com.example.goespudd.presentation.cart.adapter
...
import com.example.goespudd.databinding.ItemCartMenuBinding
import com.example.goespudd.databinding.ItemCartMenuOrderBinding
import com.example.goespudd.utils.doneEditing
...
class CartViewHolder(
    ...
    private fun setCartData(item: Cart) {
        with(binding) {
            binding.ivImgMenu.load(item.menuImgUrl) {
                crossfade(true)
            }
            tvCount.text = item.itemQuantity.toString()
    ...
    private fun setCartNotes(item: Cart) {
        binding.etNotesMenu.setText(item.itemNotes)
        binding.etNotesMenu.doneEditing {
            binding.etNotesMenu.clearFocus()
            val newItem =
                item.copy().apply {
                    itemNotes = binding.etNotesMenu.text.toString().trim()
                }
